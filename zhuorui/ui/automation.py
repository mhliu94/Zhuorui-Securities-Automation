...
from zhuorui.common.config import (
...
RETRY_ANR_TEXT = "isn't responding"
RETRY_ANR_DIALOG_TITLE = f"ZR {RETRY_ANR_TEXT}"
RETRY_ANR_TEXT_SCORE_THRESHOLD = 0.72
RETRY_ANR_DIALOG_Y_BAND = (0.30, 0.68)
RETRY_ANR_DIALOG_X_BAND = (0.07, 0.93)
RETRY_CLOSE_APP_X_RATIO = 0.35
RETRY_CLOSE_APP_Y_RATIO = 0.518
RETRY_CLOSE_APP_SETTLE_SECONDS = 0.5
...
if os.name == "nt":
    KNOWN_ADB_PATHS.append(Path.home() / "AppData" / "Local" / "Android" / "Sdk" / "platform-tools" / ADB_EXECUTABLE_NAME)
else:
...
class RetryDialogTextOcr:
    """Recognize the Android ANR title from a retry screenshot."""
    ...
    def __init__(self, font_path: Path):
        try:
            ...
            raise ZhuoruiAutomationError(
                "Pillow and numpy are required for retry-dialog screenshot OCR."
            ) from exc
        ...
        self.np = np
        self.font_path = font_path
        self.template = self.render_retry_text_template(RETRY_ANR_DIALOG_TITLE)

    @classmethod
    def from_adb(cls, adb: Adb, temp_dir: Path) -> "RetryDialogTextOcr":
        font_path = temp_dir / "Roboto-Regular.ttf"
        adb.pull(ANDROID_ROBOTO_FONT, font_path)
        return cls(font_path)

    def render_retry_text_template(self, text: str):
        font = self.ImageFont.truetype(str(self.font_path), 56)
        ...
        return self.normalize_text_mask(self.np.array(image) < 220)

    def normalize_text_mask(self, mask):
        ys, xs = self.np.where(mask)
        if len(xs) == 0:
            return None
        trimmed = mask[ys.min() : ys.max() + 1, xs.min() : xs.max() + 1]
        image = self.Image.fromarray((trimmed.astype("uint8") * 255), mode="L")
        image = image.resize((600, 80), self.Image.Resampling.NEAREST)
        return self.np.array(image) > 0

    def score_normalized_text(self, observed) -> float:
        if observed is None or self.template is None:
            return -1.0
        observed_image = self.Image.fromarray((observed.astype("uint8") * 255), mode="L")
        template_image = self.Image.fromarray((self.template.astype("uint8") * 255), mode="L")
        observed_wide = self.np.array(observed_image.filter(self.ImageFilter.MaxFilter(3))) > 0
        template_wide = self.np.array(template_image.filter(self.ImageFilter.MaxFilter(3))) > 0
        template_count = int(self.template.sum())
        observed_count = int(observed.sum())
        if template_count <= 0 or observed_count <= 0:
            return -1.0
        template_coverage = float((self.template & observed_wide).sum()) / template_count
        observed_coverage = float((observed & template_wide).sum()) / observed_count
        return (template_coverage + observed_coverage) / 2

    def recognize_retry_dialog_text(self, screenshot_path: Path) -> str:
        image = self.Image.open(screenshot_path).convert("RGB")
        width, height = image.size
        x0 = round(width * RETRY_ANR_DIALOG_X_BAND[0])
        x1 = round(width * RETRY_ANR_DIALOG_X_BAND[1])
        y0 = round(height * RETRY_ANR_DIALOG_Y_BAND[0])
        y1 = round(height * RETRY_ANR_DIALOG_Y_BAND[1])
        array = self.np.array(image.crop((x0, y0, x1, y1)))
        dark = (
            (array[:, :, 0] < 180)
            & (array[:, :, 1] < 180)
            & (array[:, :, 2] < 180)
        )
        active_rows = self.np.where(dark.sum(axis=1) >= 6)[0].tolist()
        for top, bottom in contiguous_runs(active_rows, max_gap=4):
            if bottom - top + 1 < 12:
                continue
            line = dark[max(0, top - 2) : min(dark.shape[0], bottom + 3), :]
            observed = self.normalize_text_mask(line)
            if self.score_normalized_text(observed) >= RETRY_ANR_TEXT_SCORE_THRESHOLD:
                return RETRY_ANR_DIALOG_TITLE
        return ""
...
class ZhuoruiTrader:
    ...
    def retry_close_app_point(self) -> tuple[int, int]:
        width, height = self.adb.wm_size()
        return round(width * RETRY_CLOSE_APP_X_RATIO), round(height * RETRY_CLOSE_APP_Y_RATIO)

    def close_anr_dialog_and_restart_on_retry(self) -> bool:
        with tempfile.TemporaryDirectory(prefix="zhuorui-retry-anr-") as temp_name:
            temp_dir = Path(temp_name)
            screenshot_path = temp_dir / "retry-screen.png"
            self.adb.screenshot(screenshot_path)
            ocr = RetryDialogTextOcr.from_adb(self.adb, temp_dir)
            visible_text = ocr.recognize_retry_dialog_text(screenshot_path)

        if RETRY_ANR_TEXT not in visible_text.casefold():
            return False

        print(
            f"Retry screenshot OCR detected {RETRY_ANR_DIALOG_TITLE!r}; tapping Close app before restart.",
            file=sys.stderr,
            flush=True,
        )
        self.adb.tap(*self.retry_close_app_point())
        time.sleep(RETRY_CLOSE_APP_SETTLE_SECONDS)
        self.launch()
        self.wait_for_order_retry_landing()
        return True

    def prepare_for_retry(self, restart_if_responsive: bool = False) -> None:
        if self.close_anr_dialog_and_restart_on_retry():
            return
        if restart_if_responsive:
            self.restart_app_after_retry_check()

    def restart_app_after_retry_check(self) -> None:
    ...
    def restart_app_for_order_retry(self) -> None:
        ...
        self.prepared_limit_price = None
        self.market_reference_price = None
        self.prepare_for_retry(restart_if_responsive=True)
