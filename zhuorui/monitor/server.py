"""Local dashboard for the Windows API listener or Linux emulator listener."""
...
class ZhuoruiController:
    """Control the API listener independently from its session-source emulator."""
    ...
    def __init__(
        ...
        *,
        ...
        if backend not in {"api", "ui"}:
            raise ValueError("Listener backend must be api or ui.")
        self.root = root.resolve()
        self.backend = backend
        self.probe = probe
        self.runner = runner
        self.control_runner = control_runner
        listener_name = "zhuorui_api_listener" if backend == "api" else "zhuorui_listener"
        self.pid_path = self.root / f"{listener_name}.pid"
        self.run_path = self.root / f"{listener_name}.current.json"
    ...
    def public_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            live_orders_enabled = config_bool(api, "live_orders_enabled", True) and trading if self.backend == "api" else trading
    ...
    def script_status(self, now: datetime | None = None) -> dict[str, Any]:
        ...
        if pid is None:
            return {
                ...
                "backend": self.backend,
                "message": pid_error or f"The {self.backend.upper()} trading listener is not running.",
            }

        running, started_at = self._tracked_process(pid, metadata, self.probe)
        if self.backend == "api" and (
            metadata.get("backend") != "api"
            or metadata.get("pid") not in (pid, str(pid))
            or not parse_datetime(metadata.get("started_utc"))
        ):
            running = False
        if not running:
            return {
                ...
                "duration_seconds": None,
                "backend": self.backend,
                "message": f"The {self.backend.upper()} listener is not running; PID {pid} is stale or unverified.",
            }
        ...
        status = {
            ...
            "backend": self.backend,
            "live_orders_enabled": metadata.get("live_orders_enabled") is True if self.backend == "api" else self.public_config()["live_orders_enabled"],
            "message": f"The {self.backend.upper()} trading listener process is running.",
        }
        if self.backend == "api":
            state_path = metadata.get("state_file")
            state = read_json(Path(state_path)) if isinstance(state_path, str) and state_path else {}
            updated_at = parse_datetime(state.get("updated_at"))
            current_state = updated_at and started_at and started_at - timedelta(seconds=1) <= updated_at <= now + timedelta(seconds=60)
            if current_state:
                status.update({
                    "live_orders_enabled": state.get("live_orders_enabled") is True,
                    "session_status": state.get("session_status"),
                    "last_holdings_publish": state.get("last_holdings_publish"),
                    "last_error": state.get("last_error"),
                })
                if (now - updated_at).total_seconds() > 90:
                    status.update(state="attention", message="The API process is running, but its status update is overdue.")
                elif state.get("last_error"):
                    status.update(state="attention", message=str(state["last_error"]))
                else:
                    last_publish = parse_datetime(state.get("last_holdings_publish"))
                    interval = state.get("holdings_interval_seconds", 30)
                    overdue_after = max(90, float(interval) * 3) if isinstance(interval, (int, float)) else 90
                    if (now - (last_publish or started_at)).total_seconds() > overdue_after:
                        status.update(state="attention", message="The API listener is running, but holdings publication is overdue.")
                    elif last_publish:
                        status["message"] = "The API listener is running and has published account holdings."
            elif duration is not None and duration > 90:
                status.update(state="attention", message="The API process is running; waiting for a current listener status update.")
        return status
    ...
    def health_status(self, emulator: Mapping[str, Any], checked_at: datetime) -> dict[str, Any]:
        ...
        overall_metrics = (
            [item for item in metrics if item["id"] in {"machine_cpu", "machine_memory"}]
            if self.backend == "api" else metrics
        )
        overall_level = max(overall_metrics, key=lambda item: HEALTH_LEVEL_RANK[item["level"]])["level"]
        summaries = {
        ...
        if self.backend == "api":
            summaries = {
                "healthy": "Windows resources are within normal ranges. Android readings describe the session source only.",
                "under_load": "Windows resources are elevated. Android readings do not determine API availability.",
                "restart_recommended": "Windows resource pressure needs attention. API controls do not restart the emulator.",
            }
        return {
    ...
    def _control_script(self, name: str, timeout: float = 30) -> ActionResult:
        ...
        if os.name == "nt":
            arguments = ["powershell.exe", "-NoLogo", "-NoProfile", "-NonInteractive",
                         "-ExecutionPolicy", "Bypass", "-File", str(script_path), "-Backend", self.backend]
            if name == "start_zhuorui_listener":
                arguments.extend(["-ConfigPath", str(self.config_path)])
        else:
            if self.backend != "ui":
                return ActionResult(False, "API session storage currently requires Windows. Use the UI listener on Linux.")
    ...
    def stop_script(self) -> ActionResult:
        ...
        return self._control_script("stop_zhuorui_listener", timeout=40)

    def restart_script(self) -> ActionResult:
        stopped = self.stop_script()
        if not stopped.ok:
            return stopped
    ...
    def perform(self, action: str) -> ActionResult:
        actions = {
            ...
            "script/restart": self.restart_script,
